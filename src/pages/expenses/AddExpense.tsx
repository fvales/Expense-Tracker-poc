...
type AddExpenseFormValues = Omit<AddExpenseType, 'date'> & { date: string };
...
const AddExpense: FC<IAddExpenseProps> = ({
    ...
    const validationSchema: yup.ObjectSchema<AddExpenseFormValues> = yup.object(
        {
            description: yup
                .string()
                .required(
                    t('fieldIsMandatory', {
                        field: t('addExpense.description'),
                    })
                )
                .max(255),
            amount: yup
                .number()
                .required(
                    t('fieldIsMandatory', { field: t('addExpense.amount') })
                )
                .min(0),
            date: yup
                .string()
                .required(
                    t('fieldIsMandatory', { field: t('addExpense.date') })
                ),
            category: yup
                .string<Category>()
                .required(
    ...
    return (
        <PopupDialog
            open={open}
            title={isEditMode ? t('editExpense.title') : t('addExpense.title')}
            onClose={onClose}
